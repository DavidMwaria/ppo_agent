...
def validate_episode(env, env_HR, env_RH, sch_lst, pth_h, ptn_h):
    ...
    while True:
        ...
        if not pth_h:
            path_action, probs, value = pth_sch.agent.choose_action(obs)
            path_action_HR = h_path(env_HR)
            path_action_RH, probs_RH, value_RH = pth_sch.agent.choose_action(obs_RH)
            pth_sch.sub_step += 1
        else:
        ...
        if not ptn_h and path_action_HR != 0:
            ...
            pattern_action_HR = ptn_sch.agent.sample(obs_HR)
