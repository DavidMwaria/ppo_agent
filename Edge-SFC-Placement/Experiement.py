...
from PPO_implementation import *
...
def train_model(sch_lst,memory_warmup_size, learn_freq, batch_size):
    ...
    for k in sch_lst.sch_dict.keys():
        if len(sch_lst.sch_dict[k].PPOMemory) > memory_warmup_size and (sch_lst.sch_dict[k].sub_step % learn_freq == 0):
    ...
    while True:
        ...
        if not env.use_path_h :
            ...
            if sch_lst.all_warmup:
                path_action = pth_sch.agent.choose_action(obs)
                #print(">A<")
                #print(path_action)
            else:
        ...
        '''pattern selection'''
        if not env.use_pattern_h and path_action[0]!=0 :
            ptn_sch = get_scheduler(env, path_action[0], sch_lst.sch_dict)
            pattern_action = ptn_sch.agent.choose_action(obs)
            if pattern_action[0] != 0:
                ptn_sch.config_ptn(pattern_action[0], env)
            ptn_sch.sub_step += 1
        elif path_action[0]!=0 :
            pattern_action =  h_pattern(env, path_action[0])
        else: # ignore pattern selection if path selection failed
            pattern_action = 0
        # deployment step
        next_obs, reward, done, _ = env.step(path_action[0])
        # append rpm
        if not env.use_path_h :
            pth_sch.PPOMemory.store_memory(state=obs, action=path_action[0], reward=reward, val=path_action[2], done=done, action_prob=path_action[1])
        if not env.use_pattern_h :
            ptn_sch.PPOMemory.store_memory(state=obs, action=path_action[0], reward=reward, val=path_action[2], done=done, action_prob=path_action[1])
