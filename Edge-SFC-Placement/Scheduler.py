...
import PPO_implementation as PPO
import numpy as np
...
class scheduler:
    def __init__(self, sch_type, mem_size, obs_dim, action_dim, lr, gamma, alpha, topo):
        self.type = sch_type
        self.PPOMemory = PPO.PPOMemory(mem_size)
        if sch_type != 'path':
        ...
        if sch_type == 'path':
            self.critic = PathCritic(obs_dim, action_dim)
        elif sch_type == '22':
...
class sch_list:
    ...
    def check_warmup(self):
        ...
        for k in self.sch_dict.keys():
            print('agent:', k, 'rpm_size:', len(self.sch_dict[k].PPOMemory))
        for k in self.sch_dict.keys():
            if len(self.sch_dict[k].PPOMemory) < self.rpm_warmup_size:
